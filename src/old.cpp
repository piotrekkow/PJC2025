...
public:
    ...
    void addRoad(Intersection* intersection1, Vector2 pos, int laneCount)
    {
        Intersection* intersection2{ addIntersection(pos) };
        Vector2 tangent{ normalizedTangent(intersection1->getPos(), pos) };
        // intersection1->updateLeg();
        intersection2->addLeg(tangent, 20.0f, laneCount, LANE_WIDTH, laneCount / 2);
    }
